...
let linkedLive='';
let liveLinkTimer:number|null=null;
...
function recordVisibleCompletion(){const ctx=context();if(!ctx||levelRecorded.has(ctx.id))return;const result=document.querySelector('.result-page h1');if(result&&text(result)==='Nível dominado!'){levelRecorded.add(ctx.id);recordStudentLevel({levelId:ctx.id,categoryId:ctx.categoryId})}}
...
function onClick(event:MouseEvent){const target=event.target as Element|null;if(!target)return;
 ...
 const start=target.closest('button');if(start){const label=text(start);if(label==='Criar Voltz Live'&&start.closest('.teacher-live-panel')){const code=text(document.querySelector('.teacher-code strong'));if(code){sessionStorage.setItem(LIVE_CLASS,code);startLiveLinking()}}if(label.includes('Começar desafio')||label.includes('Novo desafio')){firstSeen.clear();const lessonTitle=text(document.querySelector('.lesson-page h1'));const level=byTitle(lessonTitle);if(level)saveContext(level)}if(label.includes('Ver resultado'))window.setTimeout(recordVisibleCompletion,80)}
 ...
 window.setTimeout(()=>{const correct=option.classList.contains('correct')&&!option.classList.contains('wrong');recordStudentAnswer({levelId:ctx.id,categoryId:ctx.categoryId,questionKey:`${ctx.id}:${questionNo}:${q}`.slice(0,500),topic:ctx.title,correct,firstAttempt,answerIndex:optionIndex,sessionType:'individual'})},0)
}
...
function stopLiveLinking(){if(liveLinkTimer!==null){window.clearInterval(liveLinkTimer);liveLinkTimer=null}}
function startLiveLinking(){if(liveLinkTimer!==null)return;void linkPendingLive();liveLinkTimer=window.setInterval(()=>{if(!sessionStorage.getItem(LIVE_CLASS)){stopLiveLinking();return}void linkPendingLive()},1200)}

export function installLearningTelemetry(){document.addEventListener('click',onClick,true);window.addEventListener('voltz-open-view',onView);window.addEventListener('voltz-open-level',onLevel);if(sessionStorage.getItem(LIVE_CLASS))startLiveLinking();return()=>{document.removeEventListener('click',onClick,true);window.removeEventListener('voltz-open-view',onView);window.removeEventListener('voltz-open-level',onLevel);stopLiveLinking()}}
