...
async function consumeOAuthCallback(){
  ...
  if(!hash.get("access_token")||hash.get("type")==="recovery")return;
  ...
  localStorage.setItem(SESSION_KEY,JSON.stringify({access_token:accessToken,refresh_token:refreshToken,expires_at:Math.floor(Date.now()/1000)+expiresIn,user}));
}
