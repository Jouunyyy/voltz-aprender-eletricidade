...
import { liveRequest, liveUrl, setLiveUrl, type LiveConfig, type LiveResponse, type LiveState } from './live-api';
...
const isLiveState=(value:LiveResponse):value is LiveState=>'id' in value;
...
export default function VoltzLive({user,onExit,renderVisual}:{user:{id:string;name:string};onExit:(target?:string)=>void;renderVisual:(kind:NonNullable<Check['visual']>)=>ReactNode}){
 ...
 function receive(next:LiveResponse){
 ...
 useEffect(()=>{if(confirm)controls.current?.focus()},[confirm]);
