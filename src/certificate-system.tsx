...
export function CertificateFrame({certificate,frameRef,onReady}:{certificate:VoltzCertificate;frameRef:React.RefObject<HTMLIFrameElement|null>;onReady?:()=>void}){
 const render=()=>{
  const win=frameRef.current?.contentWindow as FrameWindow|null;
  if(!win?.renderVoltzCertificate)return;
...
export function CertificateViewer({code,onBack,autoPrint=false}:{code:string;onBack:()=>void;autoPrint?:boolean}){
 ...
 const ready=()=>{if(autoPrint&&!printed.current){printed.current=true;window.setTimeout(print,250)}};
