...
async function callRpc(name:string,body:Record<string,unknown>){
 ...
 if(!response.ok){const code=typeof data?.code==='string'?data.code:'RPC_ERROR';console.error('Voltz Professores RPC failed',name,response.status,code);throw new Error(`rpc:${name}:${response.status}:${code}`)}
 return data;
}
