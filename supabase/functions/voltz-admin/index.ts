...
async function rpc(name:string,body:unknown){const response=await fetch(`${SUPABASE_URL}/rest/v1/rpc/${name}`,{method:'POST',headers:{apikey:SERVICE_KEY,Authorization:`Bearer ${SERVICE_KEY}`,'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await response.json().catch(()=>null);if(!response.ok)throw new Error(`rpc:${name}:${response.status}`);return data}
async function githubStatus(){const base='https://api.github.com/repos/Jouunyyy/voltz-aprender-eletricidade';const common={headers:{Accept:'application/vnd.github+json','User-Agent':'Voltz-Admin'}};try{const [commitRes,runsRes]=await Promise.all([fetch(`${base}/commits/main`,common),fetch(`${base}/actions/runs?branch=main&event=push&per_page=1`,common)]);const commit=commitRes.ok?await commitRes.json():null;const runs=runsRes.ok?await runsRes.json():null;const run=runs?.workflow_runs?.[0]||null;return {available:Boolean(commit||run),commit:commit?{sha:String(commit.sha||'').slice(0,12),message:commit.commit?.message?.split('\n')[0]||'',at:commit.commit?.committer?.date||null}:null,pages:run?{status:run.status||null,conclusion:run.conclusion||null,at:run.updated_at||run.created_at||null}:null}}catch{return {available:false,commit:null,pages:null}}}
Deno.serve(async request=>{
 ...
 try{
  ...
  if(action==='users')data=await rpc('voltz_admin_users',{p_user:user.id,p_input:input});
   ...
   const allowed=new Set(['summary','set_teacher','schools','save_school','assign_teacher_school','feedback','reports','errors','set_item_status','emails','audit']);
 ...
 }catch(error){console.error('Voltz Admin request failed',error instanceof Error?error.message:'error');return reply({error:'Não foi possível processar o pedido administrativo.'},500)}
});
